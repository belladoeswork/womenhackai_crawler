...
class JobPostingAnalyzer:
    ...
    def load_data(self, excel_file_path):
        ...
        df['publishing_date'] = pd.to_datetime(df['publishing_date'], infer_datetime_format=True, errors='coerce')
        return df
    ...
    def time_series_analysis(self):
        daily_counts = self.data['publishing_date'].value_counts().sort_index()
        plt.figure(figsize=(12, 6))
    ...
    def experience_function_heatmap(self):
        pivot = pd.crosstab(self.data['experience'], self.data['function'])
        plt.figure(figsize=(12, 8))
        sns.heatmap(pivot, annot=True, fmt='d', cmap='YlGnBu')
        plt.title('Heatmap of Experience Levels vs Job Functions')
        plt.tight_layout()
        plt.savefig('experience_function_heatmap.png')
        plt.close()
    ...
    def generate_report(self):
        ...
        self.experience_function_heatmap()
        return report

def main():
    analyzer = JobPostingAnalyzer('Alstom_Job_Table.xlsx')
